"""
MEBU Analytics — DEEP FIELD Design System (v1.2.1 - Cloud Optimized)
...
def page_header(title, subtitle="", icon=""):
    ...
    <div style="margin-bottom:40px;">
      <h1 style="font-family:var(--font-display);letter-spacing:3px;text-transform:uppercase;margin:0;">{icon_html}{title}</h1>
      {sub_html}
      <div style="height:3px;background:linear-gradient(90deg, var(--plasma), var(--violet), transparent);margin-top:20px;"></div>
    </div>
    """
...
def temp_badge(label, value, unit="°C"):
    ...
    return f'<span style="background:rgba(255,184,0,0.1);border:1px solid var(--gold);border-radius:20px;padding:5px 15px;margin:5px;color:var(--gold);font-family:var(--font-mono);font-size:0.8rem;">{label}: {value}{unit}</span>'
